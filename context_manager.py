...
class ContextManager:
    ...
    def get_combat_context(
        self, game_engine: GameEngine, events_history: List[str]
    ) -> CombatContext:
        ...
        positioned_entities = game_engine.combat_engine.get_entities(
            game_engine.heroes, game_engine.game_data
        )
    ...
    def get_treasure_context(self, game_engine: GameEngine) -> TreasureContext:
        curr_encounter = game_engine.movement_engine.current_encounter
        ...
        context.desc = (
            f"{treasure.name}: {treasure.description} (loot: {treasure.loot})"
        )
    ...
    def get_trap_context(self, game_engine: GameEngine) -> TrapContext:
        curr_encounter = game_engine.movement_engine.current_encounter
        context = TrapContext()
        # get heroes status
        context.heroes_status = game_engine.heroes.get_party_status()
    ...
    def get_movement_context(self, game_engine: GameEngine) -> MovementContext:
        context = MovementContext()
        # get heroes status
        context.heroes_status = game_engine.heroes.get_party_status()
        # get stress
        context.stress = game_engine.stress
        available_destinations = game_engine.movement_engine.available_destinations(
            level=game_engine.game_data
        )
        for name, idx in available_destinations:
            if name in game_engine.game_data.rooms.keys():
                ...
                context.encounters_desc.append(
                    [ContextManager.__get_encounter_description(area.encounter)]
                )
            elif name in game_engine.game_data.corridors.keys():
                area = game_engine.game_data.corridors[name]
                context.encounters_desc.append(
                    [
                        ContextManager.__get_encounter_description(encounter)
