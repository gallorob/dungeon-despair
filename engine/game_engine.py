...
class GameEngine:
    def __init__(self, heroes_player: Player, enemies_player: Player):
    ...
    def set_level(self, level: Level) -> None:
        ...
        self.move_to(dest=Destination(to=self.scenario.current_room, idx=-1))

    def restart_level_from_room(self, room_name: str) -> None:
    ...
    def tick(self):
        ...
        def try_combat():
            if len(self.movement_engine.current_encounter.enemies) > 0:
                ...
                ModifierSystem.apply_and_tick_modifiers(
                    self.heroes.party
                )  # Apply now in case there is a stun
                self.combat_engine.start_encounter(
                    encounter=self.movement_engine.current_encounter, heroes=self.heroes
                )  # TODO
                self.combat_engine.start_turn(
                    heroes=self.heroes,
        ...
        # Update the game state, if possible
            ...
            try_treasure()
            if self.state == GameState.INSPECTING_TRAP:
                self.state = GameState.IDLE
        elif self.state == GameState.INSPECTING_TREASURE:
            try_trap()
            if self.state == GameState.INSPECTING_TREASURE:
                self.state = GameState.IDLE
        if self.state == GameState.IN_COMBAT:
            ...
            if self.combat_engine.state == CombatPhase.END_OF_TURN:
                ...
                ModifierSystem.apply_and_tick_modifiers(
                    self.movement_engine.current_encounter.enemies
                )
                # Check for dead entities
                self.check_for_dead()
    ...
    def move_to(self, dest: Destination):
        """Move the hero party to another area of the level"""
        if self.movement_engine.current_room is None or self.movement_engine.reachable(
            level=self.scenario, dest=dest
        ):
            self.movement_engine.move_to(level=self.scenario, dest=dest)
    ...
    def player(self) -> Player:
        if self.state == GameState.IN_COMBAT:
            return (
                self.heroes_player
                if isinstance(self.combat_engine.attacker, Hero)
                else self.enemies_player
            )
        else:
            return self.heroes_player

    def process_attack(self, attack_idx: int) -> None:
        """Process the attack with the provided index"""
        self.combat_engine.process_attack(heroes=self.heroes, idx=attack_idx)

    def try_cancel_attack(self, attack_idx: int) -> None:
        """Attempt to cancel the attack with the provided index"""
        self.combat_engine.try_cancel_move(action_idx=attack_idx)

    def process_move(self, idx) -> None:
        """Move the current attacker to the specified position"""
        self.combat_engine.process_move(heroes=self.heroes, target_idx=idx)
    ...
    def check_for_dead(self) -> None:
        ...
        for hero in self.heroes.party:
            if hero.hp <= 0:
                dead_entities.append(
                    self.heroes.party.pop(self.heroes.party.index(hero))
                )
        for enemy in self.movement_engine.current_encounter.enemies:
            if enemy.hp <= 0:
                dead_entities.append(
                    self.movement_engine.current_encounter.enemies.pop(
                        self.movement_engine.current_encounter.enemies.index(enemy)
                    )
                )
        if self.state == GameState.IN_COMBAT:
    ...
    def process_disarm(self) -> None:
        ...
        self.actions_engine.resolve_trap_encounter(
            encounter=self.movement_engine.current_encounter, heroes=self.heroes
        )

    def process_looting(self, choice: LootingChoice) -> None:
        ...
        if choice == LootingChoice.LOOT or choice == LootingChoice.INSPECT_AND_LOOT:
            self.actions_engine.resolve_treasure_encounter(
                treasure=treasure, hero=hero, encounter=encounter, choice=choice
            )
        else:
            msg_system.ignore_looting(hero=hero, treasure=treasure)
            stress_system.process_ignore_looting(hero=hero, treasure=treasure)

    def targeted(self, idx: int) -> List[int]:
    ...
    def check_wave_over(self) -> None:
        if len(self.heroes.party) == 0:
            ...
            msg_system.add_msg(
                f"<b>Wave #{self.wave + 1} is over</b>: all heroes are dead!"
            )
