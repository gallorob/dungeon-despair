...
def get_entities_differences(
    ref_level: Level, curr_level: Level
    ...
    for room_name in ref_level.rooms.keys():
        ref_enemies = [
            f"{x.name}__{x.hp}" for x in ref_level.rooms[room_name].encounter.enemies
        ]
        curr_enemies = [
            f"{x.name}__{x.hp}" for x in curr_level.rooms[room_name].encounter.enemies
        ]
        ...
        ref_treasures = [
            f"{x.name}" for x in ref_level.rooms[room_name].encounter.treasures
        ]
        curr_treasures = [
            f"{x.name}" for x in curr_level.rooms[room_name].encounter.treasures
        ]
    ...
    for corridor_name in ref_level.corridors.keys():
        for i, encounter in enumerate(ref_level.corridors[corridor_name].encounters):
            ...
            curr_enemies = [
                f"{x.name}__{x.hp}"
                for x in curr_level.corridors[corridor_name].encounters[i].enemies
            ]
            for j, ref_enemy in enumerate(ref_enemies):
                if ref_enemy not in curr_enemies:
                    diff_entities.append(
                        ref_level.corridors[corridor_name].encounters[i].enemies[j]
                    )
                    locations.append(corridor_name)
            ref_traps = [f"{x.name}" for x in encounter.traps]
            curr_traps = [
                f"{x.name}"
                for x in curr_level.corridors[corridor_name].encounters[i].traps
            ]
            for j, ref_trap in enumerate(ref_traps):
                if ref_trap not in curr_traps:
                    diff_entities.append(
                        ref_level.corridors[corridor_name].encounters[i].traps[j]
                    )
                    locations.append(corridor_name)
            ref_treasures = [f"{x.name}" for x in encounter.treasures]
            curr_treasures = [
                f"{x.name}"
                for x in curr_level.corridors[corridor_name].encounters[i].treasures
            ]
            for j, ref_treasure in enumerate(ref_treasures):
                if ref_treasure not in curr_treasures:
                    diff_entities.append(
                        ref_level.corridors[corridor_name].encounters[i].treasures[j]
                    )
                    locations.append(corridor_name)
...
def reset_entity(
    ref_level: Level, curr_level: Level, entity: Entity, location: str
) -> None:
    def add_or_replace_entity(
        entity: Entity, encounter: Encounter, entity_t: Optional[EntityEnum] = None
    ) -> None:
        if entity_t is None:
            if isinstance(entity, Enemy):
                entity_t = EntityEnum.ENEMY
            elif isinstance(entity, Trap):
                entity_t = EntityEnum.TRAP
            else:
                entity_t = EntityEnum.TREASURE
    ...
    if location in curr_level.rooms.keys():
        add_or_replace_entity(
            entity=entity, encounter=curr_level.rooms[location].encounter
        )
    else:
        for i, encounter in enumerate(curr_level.corridors[location].encounters):
            ...
            ref_names = [
                x.name
                for x in ref_level.corridors[location]
                .encounters[i]
                .entities[entity_t.value]
            ]
            if entity.name in ref_names:
                add_or_replace_entity(
                    entity=entity, encounter=encounter, entity_t=entity_t
                )
