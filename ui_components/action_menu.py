...
trap_choices = [Choice(name='Attempt to disarm', description='Try your luck disarming the trap')]
treasure_choices = [Choice(name='Inspect and Loot', description='Inspect the treasure for traps and then loot', looting_choice=LootingChoice.INSPECT_AND_LOOT),
                    Choice(name='Loot', description='Try your luck looting the treasure', looting_choice=LootingChoice.LOOT),
                    Choice(name='Ignore', description='Move on and leave the treasure behind', looting_choice=LootingChoice.IGNORE)]
...
class ActionWindow(UIWindow):
	...
	def display_actions(self,
	                    actions: List[Union[Attack, Choice]],
	                    disable_not_moving: bool = False):
		self.clear_actions()
		
		btn_height = self.get_container().rect.height / 8
		starting_height = self.get_container().rect.height / 2 - (btn_height * len(actions)) / 2
		
		for i, action in enumerate(actions):
			action_btn = UIButton(text=action.name,
			...
			if isinstance(action, Attack):
				...
				if disable_not_moving and get_enum_by_value(ActionType, action.type) != ActionType.MOVE:
					action_btn.disable()
			else:
