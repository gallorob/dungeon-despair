...
class HeroRosterWindow(UIWindow):
    def __init__(self, rect: pygame.Rect, ui_manager: IUIManagerInterface):
        super().__init__(rect, ui_manager, window_display_title='Heroes Roster', resizable=False, draggable=False)
        ...
        self.heroes_portraits_block = (self.relative_rect.width - (2 * self.heroes_portraits_hoff)) / 4
        self.heroes_portraits_innerpad = (self.heroes_portraits_block / 4) / 4
        self.hero_portrait_w = self.heroes_portraits_block - 2 * self.heroes_portraits_innerpad
        self.hero_portrait_h = self.log_panel_voff - 2 * self.heroes_portraits_voff

        self.log_panel = UITextBox(
            html_text="<b>Events log: </b>",
            relative_rect=pygame.Rect(0, self.log_panel_voff,
                                      self.relative_rect.width, self.log_panel_height),
            manager=ui_manager,
    ...
    def generate_party_steps(self, wave_n: int) -> bool:
        ...
        self.num_heroes, self.n_attacks, self.difficulty = scale_difficulty(wave_n=wave_n)
        ...
        for i in range(self.num_heroes):
            hero_portrait = UIImage(
				relative_rect=Rect(
                    self.heroes_portraits_hoff + i * (self.hero_portrait_w + 2 * self.heroes_portraits_innerpad) + self.heroes_portraits_innerpad,
        ...
        for i in range(self.num_heroes):
            ...
            self.__args.append({'msg': f"Generating hero {i + 1} / {self.num_heroes}..."})
            
            self.__ops.append(generate_hero)
            self.__args.append({'n_attacks': self.n_attacks, 'difficulty': self.difficulty, 'curr_heroes': []})

            self.__ops.append(self.update_log_text)
            self.__args.append({'msg': "New hero created ({hero_name})! Generating sprite..."})
    ...
    def step(self) -> None:
        if self.__step < len(self.__ops):
            ...
            elif self.__last_res is not None and isinstance(self.__last_res, Hero) and op == generate_sprite:
            ...
            res = op(**args)
            if isinstance(res, Hero): self.__last_res = res
    ...
    def update_portrait(self, idx: int) -> None:
        self.heroes[idx].set_image(pygame.image.load(os.path.join(configs.assets.dungeon_dir, self.__last_res.sprite)))
