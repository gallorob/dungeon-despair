...
class Checkbox(UIButton):
    def __init__(
        ...
        super().__init__(
            text="", relative_rect=relative_rect, manager=manager, container=container
        )
        self.enable()
        self.ticked = "X"
        self.unticked = " "
    ...
    def process_event(self, event):
        if (
            ...
            and self.get_abs_rect().collidepoint(event.pos)
        ):
...
class RegenPicker(UIWindow):
    def __init__(
        ...
        super().__init__(
            rect,
            ui_manager,
            window_display_title="Pick Regenerables",
            resizable=False,
            draggable=False,
            always_on_top=True,
        )
        ...
        self.diff_entities, self.locations = get_entities_differences(
        ...
        self.panel = UIPanel(
            Rect(
                0,
                self.get_container().rect.height / 10,
                self.get_relative_rect().width,
                self.get_relative_rect().height - self.get_container().rect.height / 5,
            ),
            starting_height=1,
        ...
        self.regen_button = UIButton(
            Rect(
                0,
                self.get_relative_rect().height - self.get_container().rect.height / 5,
                self.get_relative_rect().width,
                self.get_relative_rect().height / 10,
            ),
            "Regenerate",
            self.ui_manager,
            container=self,
    ...
    def create_entity_grid(self):
        ...
        for i, (entity, location) in enumerate(zip(self.diff_entities, self.locations)):
            ...
            entity_image = pygame.transform.scale(
                entity_image,
                (entity_image.get_width() * ratio, entity_image.get_height() * ratio),
            )
            image_label = UIImage(
                Rect(
                    (40, y_offset + i * entity_image.get_height()),
                    (entity_image.get_width(), entity_image.get_height()),
                ),
                entity_image,
                self.ui_manager,
            ...
            name_label = UILabel(
                Rect((90, y_offset + i * entity_image.get_height()), (200, 40)),
                f"{entity.name}",
                self.ui_manager,
                container=self.panel,
            )
            location_label = UILabel(
                Rect((300, y_offset + i * entity_image.get_height()), (200, 40)),
                f"{location}",
                self.ui_manager,
                container=self.panel,
            )
            cost_label = UILabel(
                Rect((510, y_offset + i * entity_image.get_height()), (100, 40)),
                f"Cost: {entity.cost}",
                self.ui_manager,
                container=self.panel,
            )
            ...
            checkbox = Checkbox(
                Rect(
                    (
                        10,
                        y_offset
                        + i * entity_image.get_height()
                        + (entity_image.get_height() / 2),
                    ),
                    (20, 20),
                ),
                self.ui_manager,
    ...
    def regenerate_entities(self):
        ...
        for i, (entity, location) in enumerate(zip(self.diff_entities, self.locations)):
            if self.checkboxes[i].text == self.checkboxes[i].ticked:
                reset_entity(
                    ref_level=self.level_copy,
                    curr_level=self.game_engine.scenario,
                    entity=entity,
                    location=location,
                )
