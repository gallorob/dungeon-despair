...
class EncounterPreview(UIWindow):
    def __init__(self, rect: Rect, ui_manager: IUIManagerInterface):
        super().__init__(
            rect,
            ui_manager,
            window_display_title="Encounter",
            resizable=False,
            draggable=False,
        )
        ...
        if self.stats_level is None:
            self.stats_level = UILabel(
                relative_rect=Rect(
                    ...
                    0,
                    self.get_container().rect.width / 3,
    ...
    def display_corridor_background(self, corridor: Corridor, idx: int):
        ...
        before_image = pygame.image.load(
            os.path.join(configs.assets.dungeon_dir, corridor.sprites[idx])
        )
        area_image = pygame.image.load(
            os.path.join(configs.assets.dungeon_dir, corridor.sprites[idx + 1])
        )
        after_image = pygame.image.load(
            os.path.join(configs.assets.dungeon_dir, corridor.sprites[idx + 2])
        )

        scale_factor = self.get_container().rect.height / area_image.get_height()
        new_width, new_height = int(scale_factor * area_image.get_width()), int(
            scale_factor * area_image.get_height()
        )
        ...
        before_image = before_image.subsurface(
            Rect(
                diff_w / 2,
                0,
                before_image.get_width() - (diff_w / 2),
                before_image.get_height(),
            )
        )
        after_image = after_image.subsurface(
            Rect(0, 0, diff_w / 2, after_image.get_height())
        )

        combined_surface = pygame.Surface(
            (
                before_image.get_width()
                + area_image.get_width()
                + after_image.get_width(),
        ...
        combined_surface.blit(
            after_image, (before_image.get_width() + area_image.get_width(), 0)
        )
    ...
    def display_room_background(self, room: Room):
        ...
        self.stats_level = None
        area_image = pygame.image.load(
            os.path.join(configs.assets.dungeon_dir, room.sprite)
        )
        scale_factor = min(
            self.get_container().rect.width / area_image.get_width(),
            self.get_container().rect.height / area_image.get_height(),
        )
        area_image = pygame.transform.scale(
            area_image,
            (
                int(scale_factor * area_image.get_width()),
    ...
    def display_encounter(self, encounter: Encounter):
        ...
        def __add_and_show_entities(
            max_n: int, entity_type: str, height_diff: int, additional_x_offset: float
        ) -> List[UIImage]:
            sprites = []
            ...
            for i, entity in enumerate(encounter.entities[entity_type]):
                entity_image = pygame.image.load(
                    os.path.join(configs.assets.dungeon_dir, entity.sprite)
                )
        ...
        # show enemies
        self.enemies = __add_and_show_entities(
            max_n=ddd_config.max_enemies_per_encounter,
            entity_type="enemy",
            height_diff=1,
            additional_x_offset=0,
        )
        # show traps
        self.traps = __add_and_show_entities(
            max_n=4,  # ddd_config.max_traps_per_encounter,
            entity_type="trap",
            height_diff=1,
            additional_x_offset=-self.padding,
        )
        # show treasures
        self.treasures = __add_and_show_entities(
            max_n=4,  # ddd_config.max_treasures_per_encounter,
    ...
    def display_heroes(self, heroes: HeroParty):
        ...
        for i, hero in enumerate(heroes.party):
            hero_image = pygame.image.load(
                os.path.join(configs.assets.dungeon_dir, hero.sprite)
            )
    ...
    def update_attacking(self, idx: int):
        ...
        self.attacking = UIImage(
            relative_rect=Rect(
                ref_sprite.relative_rect.x
                + ref_sprite.relative_rect.width / 2
                - self.padding / 4,
    ...
    def update_targeted(self, idxs: List[int]):
        ...
        for idx in idxs:
            ...
            y_offset = (
                (self.background_image.rect.height / 2)
                - (ref_sprite.relative_rect.height / 2) / 2
                - self.padding / 4
            )
            self.targeted.append(
                UIImage(
                    relative_rect=Rect(
                        ref_sprite.relative_rect.x
                        + ref_sprite.relative_rect.width / 2
                        ...
                        (
                            y_offset
                            if (attacking_x != ref_sprite.rect.x)
                            else y_offset - self.padding / 2
                        ),
                        self.padding / 2,
                        self.padding / 2,
                    ),
    ...
    def update_modifiers(self, heroes: HeroParty, enemies: List[Enemy]):
        ...
        for i, hero in enumerate(heroes.party):
            for j, modifier in enumerate(hero.modifiers):
                ...
                y_offset = (
                    (self.background_image.rect.height / 2)
                    + ref_sprite.relative_rect.height
                    + self.padding / 5
                )
                modifier_icon = UIImage(
                    ...
                    image_surface=pygame.image.load(
                        get_icon(get_enum_by_value(ModifierType, modifier.type))
                    ),
                    manager=self.ui_manager,
                    container=self.get_container(),
                    parent_element=self,
