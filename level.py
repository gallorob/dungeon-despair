...
class Level(BaseModel):
	...
	def save_to_file(self, filename: str, conversation: str = '') -> None:
		# get all images
		all_images = os.listdir(configs.temp_dir)
		images = {image_path: PIL.Image.open(os.path.join(configs.temp_dir, image_path)) for image_path in all_images}
		bin_data = {
			'level':        self,
			'images':       images,
	...
	def load_from_file(filename: str) -> Tuple["Level", str]:
		with open(filename, 'rb') as f:
			...
			for fpath, image in images.items():
				image.save(os.path.join(configs.temp_dir, fpath))
